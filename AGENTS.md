...
The script resolves the harness (`fm-harness.sh crew`), owns the verified launch templates, applies contextual Codex MCP launch overrides for non-agent-native projects and secondmate homes, resolves the project's delivery mode (`fm-project-mode.sh`) for ship/scout tasks, and records `harness=`, `kind=`, `mode=`, and `yolo=` in the task's meta; a non-flag third argument containing whitespace is treated as a raw launch command (only for verifying new adapters).
