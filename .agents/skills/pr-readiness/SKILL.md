...
1. Confirm the intended target and audience before any public action.

   Do not let `origin`, the currently checked-out branch, an existing PR, or the
   no-mistakes default decide the target silently. Write down:
   - action: open / update / comment / present / ask to merge / merge;
   - target repository and branch;
   - head repository and branch;
   - audience: private fork/local owner or upstream maintainer.

   If the user asked to land work in "our fork", "my fork", "local main", or a
   project configured as local-only, do not open or update an upstream PR unless
   the user explicitly changes the target to that upstream repository.

   For any target that could notify an upstream maintainer, state the target in
   the captain-facing update before acting. If the user did not explicitly name
   that upstream target, stop and ask one direct question.

   Use `gh-axi`, not raw `gh`, in this repo.
   ```sh
   BRANCH="$(git branch --show-current)"
   CURRENT_REPO="$(gh-axi repo view --json nameWithOwner --jq '.nameWithOwner')"
   ORIGIN_URL="$(git remote get-url origin 2>/dev/null || true)"
   git remote -v
   git branch -vv
   ```

2. Refresh the base facts for the actual action.
   ...
   that base directly. For fork, replacement, or local-fork workflows, set
   `TARGET_BASE_REPO` to the intended target repository rather than assuming
   `origin`.
   ```sh
   BRANCH="$(git branch --show-current)"
   if [ -z "${TARGET_BASE_REPO:-}" ]; then
     echo "TARGET_BASE_REPO is required before pre-PR readiness checks" >&2
     exit 2
   fi
   BASE_REPO="$TARGET_BASE_REPO"
...
3. Compare against current base, not the branch's old base.
   ```sh
...
4. Check overlap and supersession.
...
- The target repository, target branch, head repository, or audience is inferred
  rather than explicitly requested or mechanically confirmed.
- The public action would notify an upstream maintainer, but the user asked for
  a fork/local landing path or did not explicitly choose that upstream target.
